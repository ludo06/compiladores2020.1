    ...
    while True:
        ...
        for lhs, rhs, dot_idx in clos:
            if dot_idx < len(rhs):
                ...
                if simbolo in g.non_terminals:
                    for prod in g.production_rules[simbolo]:

                        novoItem = (simbolo, prod, 0)
                        new_items.add(novoItem)
...
def goto(s, x, g):
    ...
    for lhs, rhs, dot_idx in s:
        ...
        if dot_idx < len(rhs) and rhs[dot_idx] == x:
              novoItem = (lhs, rhs, dot_idx+1)
              goto_sx.add(novoItem)
...
def canonical_items(g):
    ...
    while True:
        ...
        for itens in can:
            ...
            for simbolo in g.getSymbols():

                go_to = goto(itens, simbolo, g)

                if len(go_to) > 0 and go_to not in can:
                    can.append(go_to)
    ...
    for state_l, l in enumerate(can):
        ...
        # then GOTO[i,􏰥A] = j.􏰋
                ...
                for A in g.non_terminals:
                    ...
                    go_to = goto(l, A, g)
                    if len(go_to) != 0 and go_to in can:
